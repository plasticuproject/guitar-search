...
from scrapers.reverb import scrape_reverb, dump_scrape
from scrapers.reverb_models import Listing
...
def mocked_requests_get(*args: str, **kwargs: str) -> MockResponse:
    ...
    if args[0] in electric_endpoints:
        with open("./test/scrapers/dumps/test_reverb_electric_guitars.json",
                  "r", encoding="utf-8") as infile:
            data = json.loads(infile.read())
        return MockResponse(data, 200)
    if args[0] in acoustic_endpoints:
        with open("./test/scrapers/dumps/test_reverb_acoustic_guitars.json",
                  "r", encoding="utf-8") as infile:
            data = json.loads(infile.read())
        return MockResponse(data, 200)
    if args[0] == json_error_1:
        with open("./test/scrapers/dumps/test_bad_1.json",
                  "r", encoding="utf-8") as infile:
            data = json.loads(infile.read())
        # return MockResponse(data, 200)
...
class ReverbTests(unittest.TestCase):
    ...
    with open("./test/scrapers/dumps/test_listings.json",
              "r", encoding="utf-8") as infile:
    ...
    @mock.patch("scrapers.reverb.requests.get",
